...
func TraceTransaction(c *rpc.Client, ctx context.Context, tx *types.Transaction, config *TraceConfig) ([]*Tx, error) {
	tjs, err := TracerJS()
	if err != nil {
		return nil, err
	}
	...
	err = c.CallContext(ctx, &raw, "debug_traceTransaction", tx.Hash(), config)
	if err != nil {
		return nil, err
	} else if len(raw) == 0 {
